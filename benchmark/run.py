...
gs://$GROUPING_TRAINER_BUCKET/perf/{stamp}-{run_id}/{dataset_name}/
...
# - optimum/onnxruntime/utils.py:111 maps "modernbert" -> "bert", so optimum routes ModernBERT graphs through ORT's BERT
#   optimizer (BertOnnxModel).
...
def main(
    ...
    dir_gcs_output = f"gs://{os.environ['GROUPING_TRAINER_BUCKET']}/perf/{stamp}-{run_id}/{name_dataset}"
