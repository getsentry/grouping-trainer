...
TrainingGpuType = Literal[
    tuple(gpu_type for gpu_type, config in gt.launch.gpu_type_to_config.items() if config.is_for_training)  # type: ignore[invalid-literal]
]
...
def run(
    ...
    *,
    gpu: TrainingGpuType | None = None,  # type: ignore[valid-type]
    zone: str | None = None,
