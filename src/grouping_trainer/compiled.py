...
class SentenceTransformer(gt.utils.SentenceTransformer):
    ...
    def compile_and_warm_up(self):
        ...
        # Warm up the fallback path (compiled or eager) by intentionally exceeding the biggest bucket
    ...
    def forward(self, input: dict[str, torch.Tensor], **kwargs) -> dict[str, torch.Tensor]:
        # Only use the bucketed CUDA-graph forward when both batch size and sequence length match a bucket. Out-of-
        # bucket lengths route through the dynamic-shape compiled fallback (when enabled), avoiding the cache-thrashing
        # we'd get from re-tracing the static graph for every unique length.
